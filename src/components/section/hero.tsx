    ...
    <section className="relative h-3/4 w-full min-h-96 flex justify-center items-center">
          {
              ...
              <>
                  ...
                  <div className="relative z-10 sm:px-8 md:px-24 xl:px-56 text-center py-16 lg:py-32">
                      <h1 className="mb-4 text-4xl sm:text-5xl font-bold text-white lg:text-6xl">{data?.value?.title || "[Judul hero belum diatur]"}</h1>
                      <p className="sm:mb-7 text-sm sm:text-md font-light tracking-tight  text-white lg:text-xl sm:px-16">{data?.value?.description || "[Deskripsi hero belum diatur]"}</p>
                  </div>
              </>
            )
          }
