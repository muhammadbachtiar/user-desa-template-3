  ...
  return ( 
    <>
      {   
          ...
          ) : isError && !isFetching && !logo || logo.data?.length === 0 ? (
              ...
              <>       
                <Link href={"/"} className={`flex items-center px-2 py-1 space-x-3 rtl:space-x-reverse rounded-md hover:bg-gray-300 hover:scale-105 transition transform duration-300 ease-in-out`}>
                    ...
                    <div className='flex flex-col justify-center items-center gap-1'>
                        <span className={`self-start align-baseline text-xl leading-3 tracking-tighter font-semibold uppercase text-black`}>{ logo.value?.regionEntity } </span>
                        <span className={`self-start align-baseline text-xs leading-3 font-normal italic text-black`}>{ logo.value?.regionDescription } </span>
                    </div>
                </Link>
              </>
          )
